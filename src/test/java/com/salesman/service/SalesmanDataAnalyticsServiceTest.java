...
public class SalesmanDataAnalyticsServiceTest {
	...
	@Before
	public void setUp() {
		salesmanDataAnalyticsService = new SalesmanDataAnalyticsService();
	}
	
	@Test
	public void mustProcessData() {
		List<ModelData> data = createData();
		
		DataAnalytics dataAnalytics = salesmanDataAnalyticsService.process(data);
		
		assertEquals(2, dataAnalytics.getAmountOfSalesman());
		assertEquals(1, dataAnalytics.getAmountOfClients());
		assertEquals(SALE_ID_2, dataAnalytics.getMostExpensiveSale());
		assertEquals(NAME_SALESMAN_2, dataAnalytics.getWorstSalesman());
	}

	private List<ModelData> createData() {
